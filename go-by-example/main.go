...
func main() {
	...
	var state = make(map[int]int) // 用于读写的变量
	var (
		readOps  uint64 // 统计读的次数
		writeOps uint64 // 统计写的次数
	)
	for i := 0; i < 100; i++ {
		go func() {
			total := 0
	...
	time.Sleep(time.Second)
	readOpsFinal := atomic.LoadUint64(&readOps) // 读操作的总次数
	fmt.Println("readOps:", readOpsFinal)
	writeOpsFinal := atomic.LoadUint64(&writeOps) // 写操作的总次数
	fmt.Println("writeOps:", writeOpsFinal)
	mutex.Lock()
	fmt.Println("state:", state) // 读取结果
	mutex.Unlock()
}
